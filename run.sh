...
if xmlstarlet sel -t -c '/sdf/world/physics' "$WORLD_FILE" >/dev/null 2>&1; then
	xmlstarlet ed -L -u '/sdf/world/physics/max_step_size' -v $MAX_STEP_SIZE $WORLD_FILE
	xmlstarlet ed -L -u '/sdf/world/physics/real_time_update_rate' -v $DEFAULT_UPDATE_RATE $WORLD_FILE 2>/dev/null \
		|| xmlstarlet ed -L -s '/sdf/world/physics' -t elem -n real_time_update_rate -v $DEFAULT_UPDATE_RATE $WORLD_FILE
else
	...
	xmlstarlet ed -L -s '/sdf/world/physics' -t elem -n max_step_size -v $MAX_STEP_SIZE $WORLD_FILE
	xmlstarlet ed -L -s '/sdf/world/physics' -t elem -n real_time_update_rate -v $DEFAULT_UPDATE_RATE $WORLD_FILE
...
if [[ -n "${RTF_OVERRIDE}" ]]; then
	...
	if xmlstarlet sel -t -c '/sdf/world/physics/real_time_factor' "$WORLD_FILE" >/dev/null 2>&1; then
		xmlstarlet ed -L -u '/sdf/world/physics/real_time_factor' -v ${RTF_OVERRIDE} $WORLD_FILE
		xmlstarlet ed -L -u '/sdf/world/physics/real_time_update_rate' -v ${RTF_UPDATE_RATE} $WORLD_FILE
	else
