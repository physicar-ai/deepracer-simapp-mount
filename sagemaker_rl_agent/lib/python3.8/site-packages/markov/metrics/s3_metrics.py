...
def sim_trace_log(sim_trace_dict):
    '''Logs compact step metrics to CloudWatch'''
    try:
        LOGGER.info('SIM_TRACE_LOG:%d,%d,%.4f,%.4f,%.4f,%.2f,%.2f,%s,%.4f,%s,%s,%.4f',
            sim_trace_dict.get('episode', 0),
            sim_trace_dict.get('steps', 0),
            sim_trace_dict.get('x', 0.0),
            sim_trace_dict.get('y', 0.0),
            sim_trace_dict.get('heading', 0.0),
            sim_trace_dict.get('steering_angle', 0.0),
            sim_trace_dict.get('speed', 0.0),
            sim_trace_dict.get('action', ''),
            sim_trace_dict.get('reward', 0.0),
            sim_trace_dict.get('done', False),
            sim_trace_dict.get('episode_status', ''),
            sim_trace_dict.get('progress', 0.0))
    except Exception:
        pass


# JSON step keys to include in simtrace output (excludes internal-only fields)
SIMTRACE_STEP_KEYS = {
    StepMetrics.STEPS.value,
    StepMetrics.X.value,
    StepMetrics.Y.value,
    StepMetrics.HEADING.value,
    StepMetrics.STEERING_ANGLE.value,
    StepMetrics.SPEED.value,
    StepMetrics.ACTION.value,
    StepMetrics.REWARD.value,
    StepMetrics.WHEELS_TRACK.value,
    StepMetrics.PROG.value,
    StepMetrics.CLOSEST_WAYPOINTS.value,
    StepMetrics.TRACK_LENGTH.value,
    StepMetrics.TIME.value,
    StepMetrics.EPISODE_STATUS.value,
    StepMetrics.PAUSE_DURATION.value,
    StepMetrics.DISTANCE_FROM_CENTER.value,
    StepMetrics.PROJECTION_DISTANCE.value,
    StepMetrics.TRACK_WIDTH.value,
    StepMetrics.IS_LEFT_OF_CENTER.value,
    StepMetrics.IS_REVERSED.value,
    StepMetrics.IS_CRASHED.value,
    StepMetrics.IS_OFFTRACK.value,
}


class SimtraceJsonBuffer:
    '''Buffers simtrace data in memory as episodes with steps, writes JSON to file.'''
    def __init__(self):
        self._episodes = []
        self._current_episode = None

    def start_episode(self, episode_num, waypoints, custom_boxes):
        '''Start a new episode with episode-level metadata.'''
    ...
    def add_step(self, metrics):
        '''Add a step from the step_metrics OrderedDict.'''
        if self._current_episode is None:
            self.start_episode(0, [], [])
        step = {k: v for k, v in metrics.items() if k in SIMTRACE_STEP_KEYS}
    ...
    def write_to_file(self, file_path):
        '''Write all episodes (including in-progress) to JSON file.'''
        episodes = list(self._episodes)
        if self._current_episode is not None:
            episodes.append(self._current_episode)
        with open(file_path, 'w') as f:
            json.dump(episodes, f, default=custom_serializer)
...
class TrainingMetrics(MetricsInterface, ObserverInterface, AbstractTracker):
    ...
    def __init__(self, agent_name, s3_dict_metrics, deepracer_checkpoint_json, ckpnt_dir, run_phase_sink, use_model_picker=True, 
        ...
        # JSON simtrace buffer
        self._simtrace_json_buffer = SimtraceJsonBuffer()
        self._simtrace_episode_json_buffer = SimtraceJsonBuffer() if (enable_episode_simtrace and self._simtrace_episode_s3_writer) else None
    ...
    def reset(self):
        ...
        self._episode_reward_ = 0.0
        self._progress_ = 0.0
        # Start new episode in JSON buffer
        waypoints = [list(pt) for pt in self.track_data.get_way_pnts()]
        custom_boxes = self.track_data.get_custom_boxes_reward_params('', None).get('custom_boxes', [])
        episode_num = self._episode_
        self._simtrace_json_buffer.start_episode(episode_num, waypoints, custom_boxes)
        if self._simtrace_episode_json_buffer is not None:
            self._simtrace_episode_json_buffer.reset()
            self._simtrace_episode_json_buffer.start_episode(episode_num, waypoints, custom_boxes)
    ...
    def upload_episode_metrics(self):
        # Write simtrace JSON files at episode boundary
        if self.is_save_simtrace_enabled:
            self._simtrace_json_buffer.write_to_file(self._simtrace_iteration_local_path)
            if self._simtrace_episode_json_buffer is not None:
                self._simtrace_episode_json_buffer.write_to_file(self._simtrace_episode_local_path)
    ...
    def upload_step_metrics(self, metrics):
        ...
        if not self._is_eval_:
            ...
            if self.is_save_simtrace_enabled:
                self._simtrace_json_buffer.add_step(metrics)
                if self._simtrace_episode_json_buffer is not None:
                    self._simtrace_episode_json_buffer.add_step(metrics)
    ...
    def update(self, data):
        was_eval = self._is_eval_
        self._is_eval_ = data != RunPhase.TRAIN
        # Training → Eval transition: iteration complete, reset buffer for next iteration
        if not was_eval and self._is_eval_:
            self._simtrace_json_buffer.reset()
    ...
    def update_mp4_video_metrics(self, metrics):
        ...
        self._video_metrics[Mp4VideoMetrics.THROTTLE.value] = metrics[StepMetrics.SPEED.value]
        self._video_metrics[Mp4VideoMetrics.STEERING.value] = metrics[StepMetrics.STEERING_ANGLE.value]
        ...
        # Only show Object-Avoidance obstacles ('obstacle_*') and other dynamic
        # objects on the minimap. Built-in track obstacles ('box_*') are part of
        # the track world and are excluded from the iconography.
        object_poses = [pose for object_name, pose in self.track_data.object_poses.items()\
                        if not object_name.startswith('racecar')\
                        and not object_name.startswith('box_')]
        object_locations = []
...
class EvalMetrics(MetricsInterface, AbstractTracker):
    ...
    def reset(self):
        ...
        waypoints = [list(pt) for pt in self.track_data.get_way_pnts()]
        custom_boxes = self.track_data.get_custom_boxes_reward_params('', None).get('custom_boxes', [])
        episode_num = self._number_of_trials_
        self._simtrace_json_buffer.start_episode(episode_num, waypoints, custom_boxes)
        if self._simtrace_episode_json_buffer is not None:
            self._simtrace_episode_json_buffer.reset()
            self._simtrace_episode_json_buffer.start_episode(episode_num, waypoints, custom_boxes)
