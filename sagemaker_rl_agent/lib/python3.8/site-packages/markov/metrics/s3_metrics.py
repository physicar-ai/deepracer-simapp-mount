...
class SimtraceJsonBuffer:
    ...
    def start_episode(self, episode_num, waypoints, objects):
...
class TrainingMetrics(MetricsInterface, ObserverInterface, AbstractTracker):
    ...
    def reset(self):
        ...
        objects = self.track_data.get_objects_reward_params('', None).get('objects', [])
        episode_num = self._episode_
        self._simtrace_json_buffer.start_episode(episode_num, waypoints, objects)
        if self._simtrace_episode_json_buffer is not None:
            self._simtrace_episode_json_buffer.reset()
            self._simtrace_episode_json_buffer.start_episode(episode_num, waypoints, objects)
    ...
    def update_mp4_video_metrics(self, metrics):
        ...
        # objects on the minimap. World-file objects (Custom World Builder) are part of
        ...
        object_poses = [pose for object_name, pose in self.track_data.object_poses.items()\
                        ...
                        and not self.track_data.is_world_object(object_name)]
        object_locations = []
