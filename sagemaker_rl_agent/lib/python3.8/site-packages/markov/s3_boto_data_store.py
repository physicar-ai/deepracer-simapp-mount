...
class S3BotoDataStore(DataStore):
    ...
    def _save_tf_model_to_store(self, checkpoint):
        ...
        # Get checkpoint name at the beginning before any operations
        current_checkpoint_name = None
        try:
            checkpoint_state = state_file.read()
            if checkpoint_state:
                current_checkpoint_name = str(checkpoint_state)
                LOG.info("[Checkpoint] Current checkpoint being saved: {}".format(current_checkpoint_name))
        except Exception as ex:
            LOG.info("[Checkpoint] Failed to read checkpoint state: {}".format(ex))
        ...
        # Update deepracer_checkpoints.json with the new last_checkpoint
        try:
            if current_checkpoint_name:
                LOG.info("[Checkpoint] Calling update_last_checkpoint with: {}".format(current_checkpoint_name))
                checkpoint.deepracer_checkpoint_json.update_last_checkpoint(
                    checkpoint_name=current_checkpoint_name,
                    s3_kms_extra_args=get_s3_kms_extra_args())
            else:
                LOG.info("[Checkpoint] checkpoint_name is None, skipping update_last_checkpoint")
        except Exception as ex:
            LOG.info("Failed to update last_checkpoint in JSON: {}".format(ex))
