...
def rollout_idx_to_name(rollout_idx: int) -> str:
    """Convert ROLLOUT_IDX to worker name.
    
    Args:
        rollout_idx: 0 for main, 1+ for sub workers
        
    Returns:
        'main' for idx 0, 'sub1' for idx 1, 'sub2' for idx 2, etc.
    """
    if rollout_idx == 0:
        return "main"
    return f"sub{rollout_idx}"
...
def main():
    ...
    agent_config = {
        ...
        ConfigParams.CAR_CTRL_CONFIG.value: {
            ...
            ConfigParams.BODY_SHELL_TYPE.value: rospy.get_param('BODY_SHELL_TYPE', 'deepracer'),
    ...
    # metrics_key: training-metrics/main.json, training-metrics/sub1.json, ...
    metrics_prefix = rospy.get_param('METRICS_S3_OBJECT_KEY').rsplit('/', 1)[0]  # models/{name}/metrics
    worker_name = rollout_idx_to_name(args.rollout_idx)
    metrics_key = f"{metrics_prefix}/{worker_name}.json"
    ...
    if simtrace_s3_bucket:
        ...
        # Always add worker name subfolder: training-simtrace/main, training-simtrace/sub1, ...
        worker_name = rollout_idx_to_name(args.rollout_idx)
        simtrace_s3_object_prefix = os.path.join(simtrace_s3_object_prefix, 'training-simtrace', worker_name)
        # Only enable this when the feature flag is on
        if enable_episode_simtrace:
            simtrace_episode_s3_object_prefix = os.path.join(simtrace_episode_s3_object_prefix, 'training-simtrace', worker_name)
    if mp4_s3_bucket:
