...
def set_reward_and_metrics(reward_params, step_metrics, agent_name, pos_dict, track_data,
    ...
    try:
        ...
        step_metrics[StepMetrics.DISTANCE_FROM_CENTER.value] = \
            reward_params[RewardParam.CENTER_DIST.value[0]]
        reward_params[RewardParam.PROJECTION_DISTANCE.value[0]] = \
            current_ndist * track_data.get_track_length()
        step_metrics[StepMetrics.PROJECTION_DISTANCE.value] = \
            reward_params[RewardParam.PROJECTION_DISTANCE.value[0]]
        reward_params[RewardParam.CLS_WAYPNY.value[0]] = [prev_index, next_index]
        step_metrics[StepMetrics.CLOSEST_WAYPOINTS.value] = [prev_index, next_index]
        ...
        step_metrics[StepMetrics.IS_LEFT_OF_CENTER.value] = \
            reward_params[RewardParam.LEFT_CENT.value[0]]
        ...
        step_metrics[StepMetrics.TRACK_WIDTH.value] = \
            reward_params[RewardParam.TRACK_WIDTH.value[0]]
        reward_params[RewardParam.TRACK_LEN.value[0]] = track_data.get_track_length()
        step_metrics[StepMetrics.TRACK_LENGTH.value] = track_data.get_track_length()
        step_metrics[StepMetrics.IS_REVERSED.value] = reverse_dir
        ...
        # set extra reward params for world objects (Custom World Builder placements)
        cbox_reward_params = track_data.get_objects_reward_params(agent_name, model_point)
        if cbox_reward_params:
            reward_params.update(cbox_reward_params)
...
def get_wheel_radius(version, body_shell_type='deepracer'):
    ''' Returns the wheel radius for a given physics version and body shell type
        version (float): Sim app version for which to retrieve the wheel radius
        body_shell_type (str): Body shell type (e.g. 'physicar', 'deepracer')
    '''
    if 'physicar' in str(body_shell_type).lower():
        return 0.0375
