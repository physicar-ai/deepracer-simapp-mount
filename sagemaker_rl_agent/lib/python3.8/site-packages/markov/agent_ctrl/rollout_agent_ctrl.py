...
class RolloutCtrl(AgentCtrlInterface, ObserverInterface, AbstractTracker):
    ...
    def __init__(self, config_dict, run_phase_sink, metrics):
        ...
        self._body_shell_type_ = config_dict.get(const.ConfigParams.BODY_SHELL_TYPE.value, 'deepracer')
        self._wheel_radius_ = get_wheel_radius(self._simapp_version_, self._body_shell_type_)
    ...
    def reset_agent(self):
        ...
        # Reset non-static shape objects to original positions at episode start
        self._track_data_.reset_nonstatic_cboxes()
    ...
    def _get_closest_obj(self, start_dist, name_filter=None):
        ...
        for object_name, object_pose in self._track_data_.object_poses.items():
            if object_name != self._agent_name_:
                if name_filter and name_filter not in object_name and \
                        not self._track_data_.is_world_object(object_name):
                    # World-builder objects (baked into the .world) are crash-
                    # detected like obstacles but lack the "obstacle" name
                    # prefix -- let them anchor crash resets too. (physicar)
                    continue
    ...
    def judge_action(self, agents_info_map):
        ...
        self._step_metrics_[StepMetrics.IS_CRASHED.value] = \
            reset_rules_status[EpisodeStatus.CRASHED.value]
        self._step_metrics_[StepMetrics.IS_OFFTRACK.value] = \
            reset_rules_status[EpisodeStatus.OFF_TRACK.value]
    ...
    def _judge_action_at_run_phase(self, episode_status, pause):
        ...
        if pause:
            ...
            if episode_status == EpisodeStatus.CRASHED.value:
                ...
                if 'obstacle' in self._curr_crashed_object_name or \
                        self._track_data_.is_world_object(self._curr_crashed_object_name):
                    # World-builder objects crash like obstacles but lack the
                    # "obstacle" name prefix; without this the car pauses AT the
                    # crash pose and re-crashes forever. (physicar)
        ...
        # at the finish line.
            ...
            if self._ctrl_status[AgentCtrlStatus.AGENT_PHASE.value] == AgentPhase.RUN.value:
                ...
                if 'obstacle' not in self._curr_crashed_object_name \
                        and not self._track_data_.is_world_object(self._curr_crashed_object_name):
