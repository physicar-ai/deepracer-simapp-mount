...
class ObstaclesCtrl(AgentCtrlInterface):
    def __init__(self):
        ...
        self.obstacle_type = str(rospy.get_param("OBSTACLE_TYPE", "physicar_box_obstacle"))
        ...
        self.obstacle_dimensions = ObstacleDimensions.BOT_CAR_DIMENSION if self.use_bot_car \
                                   else ObstacleDimensions.BOX_OBSTACLE_DIMENSION  # SDF 실측으로 아래서 대체
        ...
        # 박스 장애물 판정 크기 = 스폰 모델의 collision 실측 + 0.03m 버퍼
        # (구 하드코딩 상수는 어떤 박스를 골라도 동일한 크기로 판정 — 실물과 불일치)
        if not self.use_bot_car:
            try:
                import xml.etree.ElementTree as _ET
                _sz = _ET.fromstring(self.obstacle_sdf).find(".//collision//box/size").text.split()
                _dims = (float(_sz[1]) + 0.03, float(_sz[0]) + 0.03)  # (width_y, length_x) + buffer
                self.obstacle_dimensions = type("Dims", (), {"value": _dims})()
            except Exception:
                pass  # 파싱 실패 시 상수 유지
