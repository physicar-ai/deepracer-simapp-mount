...
class TrackData(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            # Auto-register obstacle models from .world file
            self._register_world_obstacles(deepracer_path)
    ...
    def _register_world_obstacles(self, deepracer_path):
        """Parse .world file and auto-register Custom World Builder models for collision detection.
        Detection contract = link name markers (model names are user-defined free names):
          <link name="object"> world object / <link name="light"> traffic light
          ('signal' = legacy marker) /
          <link name="wall"> perimeter fence (also legacy name prefix 'wall_').
        Walls get crash detection with no size buffer and are NOT exposed via the
        objects reward param. Stores original poses for non-static objects to reset.
        """
        try:
            world_name = rospy.get_param("WORLD_NAME")
            world_path = os.path.join(deepracer_path, "worlds", "{}.world".format(world_name))
            if not os.path.isfile(world_path):
                return
            tree = ET.parse(world_path)
            root = tree.getroot()
            cbox_count = 0
            self._cbox_original_poses = {}
            self._cbox_nonstatic = set()
            self._cbox_metadata = OrderedDict()
            self._circle_footprints = {}  # name -> 판정 반지름 (원기둥/구)
            for model in root.iter('model'):
                name = model.get('name', '')
                link_names = set(lk.get('name') for lk in model.findall('link'))
                is_wall = ('wall' in link_names) or name.startswith('wall_')
                is_light = ('light' in link_names) or ('signal' in link_names)  # 'signal' = 구 마커
                is_object = 'object' in link_names
                if not (is_wall or is_light or is_object):
                    continue
                # Check if static
                static_elem = model.find('static')
                is_static = static_elem is not None and static_elem.text.strip().lower() == 'true'
                # Parse pose
                ...
                # Parse collision geometry (box | cylinder | sphere).
                # link 이름이 'light'(구 'signal')면 신호등 (Custom World Builder 계약 마커)
                size_x, size_y, size_z = 0.4, 0.3, 0.3  # defaults
                geom_type = 'light' if is_light else 'box'
                box_size = model.find('.//collision//box/size')
                cyl_el = model.find('.//collision//cylinder')
                sph_el = model.find('.//collision//sphere')
                if box_size is not None and box_size.text:
                    size_parts = box_size.text.strip().split()
                    if len(size_parts) >= 3:
                        size_x, size_y, size_z = float(size_parts[0]), float(size_parts[1]), float(size_parts[2])
                    elif len(size_parts) >= 2:
                        size_x, size_y = float(size_parts[0]), float(size_parts[1])
                elif cyl_el is not None:
                    geom_type = 'cylinder'
                    _r = float(cyl_el.findtext('radius', default='0.15'))
                    size_x = size_y = 2.0 * _r  # 2D footprint = 외접 사각
                    size_z = float(cyl_el.findtext('length', default='0.3'))
                elif sph_el is not None:
                    geom_type = 'sphere'
                    _r = float(sph_el.findtext('radius', default='0.15'))
                    size_x = size_y = size_z = 2.0 * _r
                # Traffic cone: box collision footprint, kind marked via collision name
                if geom_type == 'box' and model.find(".//collision[@name='cone']") is not None:
                    geom_type = 'cone'
                # Add 0.03m buffer for collision detection (walls: no buffer —
                # tracks often run right next to the fence and must not false-crash)
                raw_size_x, raw_size_y, raw_size_z = size_x, size_y, size_z
                if not is_wall:
                    size_x += 0.03
                    size_y += 0.03
                # 원기둥/구는 원형 footprint로 판정 — 외접 사각형은 모서리에서
                # 반지름의 ~41%만큼 과대판정(공 Ø22cm 기준 +4.6cm)되기 때문
                if geom_type in ('cylinder', 'sphere'):
                    self._circle_footprints[name] = raw_size_x / 2.0 + 0.015
                # Build Pose
                obj_pose = Pose()
                obj_pose.position.x = x
                obj_pose.position.y = y
                obj_pose.position.z = z
                quat = euler_to_quaternion(roll=roll, pitch=pitch, yaw=yaw)
                obj_pose.orientation.x = quat[0]
                obj_pose.orientation.y = quat[1]
                obj_pose.orientation.z = quat[2]
                obj_pose.orientation.w = quat[3]
                # value = (Y_width, X_length) to match get_object_bounding_rect mapping
                dims = type('Dims', (), {'value': (size_y, size_x)})()
                self.initialize_object(name, obj_pose, dims)
                # Store original pose
                original_pose = Pose()
                original_pose.position.x = x
                original_pose.position.y = y
                original_pose.position.z = z
                original_pose.orientation.x = quat[0]
                original_pose.orientation.y = quat[1]
                original_pose.orientation.z = quat[2]
                original_pose.orientation.w = quat[3]
                self._cbox_original_poses[name] = original_pose
                if not is_static:
                    self._cbox_nonstatic.add(name)
                if not is_wall:  # walls are not exposed via the objects reward param
                    self._cbox_metadata[name] = {
                        'size': [raw_size_x, raw_size_y, raw_size_z],
                        'type': geom_type
                    }
                cbox_count += 1
            if cbox_count > 0:
                rospy.loginfo("TrackData: Registered {} world object(s) from world file ({} non-static)".format(
                    cbox_count, len(self._cbox_nonstatic)))
        except Exception as ex:
            rospy.logwarn("TrackData: Failed to register world obstacles: {}".format(ex))

    def is_world_object(self, name):
        '''True if the model was registered from the world file (object/light/wall).'''
        return name in getattr(self, '_cbox_original_poses', {})

    def reset_nonstatic_cboxes(self):
        """Reset non-static object_ models to their original poses in Gazebo and TrackData."""
        if not hasattr(self, '_cbox_nonstatic') or not self._cbox_nonstatic:
            return
        try:
            from markov.gazebo_tracker.trackers.set_model_state_tracker import SetModelStateTracker
            from gazebo_msgs.msg import ModelState
            for name in self._cbox_nonstatic:
                if name in self._cbox_original_poses:
                    original_pose = self._cbox_original_poses[name]
                    model_state = ModelState()
                    model_state.model_name = name
                    model_state.pose = original_pose
                    ...
                    SetModelStateTracker.get_instance().set_model_state(model_state)
                    self.object_poses[name] = original_pose
        except Exception as ex:
            rospy.logwarn("TrackData: Failed to reset non-static custom boxes: {}".format(ex))
    ...
    def get_object_reward_params(self, racecar_name, model_point, car_pose):
        ...
        Excludes object_ world objects — those are provided separately via get_objects_reward_params().
        '''
        with self._lock_:
            try:
                object_locations = [[pose.position.x, pose.position.y]
                                    for name, pose in self.object_poses.items()
                                    if racecar_name not in name and not self.is_world_object(name)]
                object_poses = [pose for name, pose in self.object_poses.items()
                                if racecar_name not in name and not self.is_world_object(name)]
    ...
    def get_objects_reward_params(self, racecar_name, model_point):
        '''Returns world objects as a list of dicts (reward param key: 'objects').
        Each dict contains: x, y, z, heading, size, type.
        '''
        if not hasattr(self, '_cbox_metadata') or not self._cbox_metadata:
            return {}
        try:
            cbox_list = []
            for name, meta in self._cbox_metadata.items():
                if name not in self.object_poses:
                    continue
                pose = self.object_poses[name]
                _, _, heading = quaternion_to_euler(
                ...
                cbox_list.append({
                    ...
                    'heading': heading,
                    'size': meta['size'],
                    'type': meta.get('type', 'box')
                })
            return {'objects': cbox_list}
        except Exception as ex:
            rospy.logwarn("TrackData: Failed to get custom boxes reward params: {}".format(ex))
            return {'objects': []}
    ...
    def get_collided_object_name(self, racecar_name):
        ...
        try:
            ...
            racecar_boundary = Polygon(object_bounding_points)
            circle_footprints = getattr(self, '_circle_footprints', {})
            for object_name in self.object_poses.keys():
                if object_name != racecar_name and object_name not in noncollidable_objects:
                    ...
                    circle_r = circle_footprints.get(object_name)
                    if circle_r is not None:
                        # 원기둥/구: 원형 footprint (회전 무관, 모서리 과대판정 없음)
                        object_boundary = Point(object_pose.position.x,
                                                object_pose.position.y).buffer(circle_r)
                    else:
                    ...
                    if racecar_boundary.intersects(object_boundary):
                        return object_name
            return ''
        except Exception as ex:
