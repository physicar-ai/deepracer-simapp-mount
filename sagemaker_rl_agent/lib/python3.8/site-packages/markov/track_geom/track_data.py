...
class TrackData(object):
    ...
    def _register_world_obstacles(self, deepracer_path):
        """Parse .world file and auto-register models named 'box_*' for collision detection.
        Uses 'box_' prefix to avoid naming conflicts with ObstaclesCtrl's 'obstacle_*' models.
        Stores original poses for non-static objects to reset after collision.
        """
        try:
            ...
            for model in root.iter('model'):
                ...
                # Parse collision geometry size (box only)
                ...
                # Add 0.1m buffer for collision detection
                ...
                cbox_count += 1
            if cbox_count > 0:
                rospy.loginfo("TrackData: Registered {} custom box(es) from world file ({} non-static)".format(
    ...
    def reset_nonstatic_cboxes(self):
        """Reset non-static box_ models to their original poses in Gazebo and TrackData."""
    ...
    def get_object_reward_params(self, racecar_name, model_point, car_pose):
        ...
        Excludes box_ custom boxes — those are provided separately via get_custom_boxes_reward_params().
        '''
        with self._lock_:
            try:
                object_locations = [[pose.position.x, pose.position.y]
                                    for name, pose in self.object_poses.items()
                                    if racecar_name not in name and not name.startswith('box_')]
    ...
    def get_custom_boxes_reward_params(self, racecar_name, model_point):
        '''Returns custom_boxes as a list of dicts with info for each box.
        Each dict contains: x, y, z, heading, size.
        '''
